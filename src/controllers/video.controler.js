...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  if (!thumbnail?.url) {
    throw new ApiError(
      500,
      "somethng went wrong during uploading the thumbnail"
    );
  }
  if (!videofile?.url) {
    throw new ApiError(
      500,
      "somethng went wrong during uploading the videofile"
    );
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        ...
        "Video thumbnail is updated successfully"
      )
    );
});
const updateVideoDetails = asyncHandler(async (req, res) => {
  ...
  if (video.owner.toString() !== req.user?._id.toString()) {
    throw new ApiError(
      404,
      "you are not eligible to change the details of this video "
    );
  }
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        { title: updatedVideo.title, description: updatedVideo.description },
...
const tooglePublishVideo = asyncHandler(async (req, res) => {
  ...
  if (video.owner.toString() !== req.user?._id.toString()) {
    throw new ApiError(
      404,
      "you are not eligible to publish or unpublish this video "
    );
  }
...
const getallvideoComments = asyncHandler(async (req, res) => {
  try {
    ...
    throw new ApiError(
      404,
      error.message || "something went wrong during fetching the comments "
    );
  }
});
